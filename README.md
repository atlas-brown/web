...
## Setup the publications workflow
1. Run `cd content/publications` and `python3 -m venv venv` 
2. Install the required packages by running `source venv/bin/activate` and `pip install bibtexparser pyyaml pygments`

## Modifying Content

After any changes, run `./build.sh` to generate HTML. 

* Make changes in the `/content/` folder using Markdown for all pages apart from **publications**. Use the .bib instead and make sure url is not empty!
